...
Rectangle{
                        ...
                        && resp.result.detail.command)
                {
                    ...
                    if((typeof content) === "string")
                    {
                        ...
                        for (var i = 0; i < records.length; ++i)
                        {
                            ...
                            if(i == 0){
                                flagGroup1Id = l[0];
                                queryAddedGroup(flagGroup1Id, true);
    ...
    function addGroups(groupName)
    {
        ...
        if(!(Logic.post(work.target, path, xml, handlers)))
            messageDialog("数据请求失败,再试一次！", 4);
    }
    ...
    function queryAddedGroup(idNumber, isDrag)
    {
    ...
    function queryGroupUser(groupid)
    {
        ...
        //  if(indexFlag == 0){
        //availableUserModel.clear(); //注释此句，可以保证向下拖拽的时候不会闪烁
        Logic.query(work.target, path, handlers);
...
//                                        allUsersModel_p.append(allUsersModel.get(i));
                                }
    ...
    function searchEditHandle(dat)
    {
            var ncout = 6;
            if(menuListView.currentIndex == 0)
            {
                availableUserModel.clear();
                for(var i = 0; i < allUsersModel_p.count; ++i)
                    availableUserModel.append(allUsersModel_p.get(i));
                while(ncout--)
                    for(var i = 0; i < availableUserModel.count; i++)
                    {
                        var userData = availableUserModel.get(i);
                        //if(userData.realName !== dat)
                        var str = new String(userData.realName);
                        if(str.indexOf(dat) == -1)
                        {
                            availableUserModel.remove(i);
                        }
                    }
                //if(0 == availableUserModel.count)
                //{
                    //messageDialog("可用用户中没有该用户！", 4);
                    //queryGroupUser((groupUserView.model.get(groupUserView.currentIndex).id));
                //}
            }
            else{
                //console.log("menuListView.currentIndex==1 " + allUsersModel.count);
                for(var n = 0; n < allUsersModel.count ; n++){
                    var userDat = allUsersModel.get(n);
                    //console.log("realName " + allUsersModel.get(i).realName);
                    if(userDat.realName === dat || userDat.username === dat)
                    {
                        console.log("search entered! " + allUsersModel.count);
                        var data = allUsersModel.get(n);
                        ...
                        Item {
                            ...
                            Image{
                                ...
                                MouseArea{
                                    ...
                                    //控制可拖拽的区域
                                    drag.minimumX: column1.width + 20;
                                    drag.minimumY: 220;
                                    drag.maximumY: 910//column2.width //+ column4.width - 300
                    ...
                    DropArea{
                        ...
                        onDropped: {
                            ...
                            if(avaiableUserView.drag1ItemIndex !== -1 && flagDropCtrl == 2){
                                ...
                                queryAddedGroup(groupUserModel.get(groupUserView.currentIndex).id, true);
                            ...
                            Rectangle{
                                ...
                                ReComboBox{
                                    ...
                                    onChoiceSip: {
                                        ...
                                        menuModel.setProperty(1, "name", "Sip单机全部用户");
                                    }
                                    onChoiceVir: {
                                        phyType.visible = false;
                                        audioAlg.visible = false;
                                        ...
                                        input2.text = "";
                                        input3.text = "";
                                        menuModel.setProperty(1, "name", "虚拟单机全部用户");
                                    }
                                    onChoicePhy: {
                                        ...
                                        input3.text = "";
                                        //input4.text = "";
                                        menuModel.setProperty(1, "name", "物理单机全部用户");
                                    }
                                    ...
                                    onChoiceCon: {
                                        ...
                                        input3.text = "";
                                        //input4.text = "";
                                        menuModel.setProperty(1, "name", "联系人全部用户");
                                    }
                                }
                                ...
                                Rectangle{
                                    ...
                                    TextField{
                                        ...
                                        placeholderText:menuListView.currentIndex == 0 ?  "输入用户姓名" : "输入用户姓名/用户名";
                                        font.pixelSize: 13;
                                        style: TextFieldStyle {
                                            background: Rectangle {
                                                radius: 8
                                        ...
                                        onTextChanged: {
                                            ...
                                            if(dat){
                                                searchEditHandle(dat);
                                            }
                                            else if(menuListView.currentIndex == 0)
                                            {
                                                 queryGroupUser((groupUserView.model.get(groupUserView.currentIndex).id));
                                            }
                                            else
